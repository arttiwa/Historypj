...
func GetRoom_has_Device(c *gin.Context) {
	var room_has_device entity.Room_has_Device
	id := c.Param("id")
	if err := entity.DB().Preload("User").Preload("Room").Preload("Room.Building").Preload("Device").Raw("SELECT * FROM room_has_devices WHERE id = ?", id).Find(&room_has_device).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"data": room_has_device})
